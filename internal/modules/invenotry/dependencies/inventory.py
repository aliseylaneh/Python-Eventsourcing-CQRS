...
from internal.modules.invenotry.commands.inventory import CompleteReservedStockCommand, CreateInventoryCommand, ReserveStockCommand, \
...
async def inventory_event_repository(collection=Depends(get_inventory_events_collection)) -> IInventoryRepository:
...
async def get_create_inventory_command(
        aggregate: Type[AggregateRoot] = Depends(inventory_aggregate),
        event_repository: IInventoryRepository = Depends(
            inventory_event_repository)
) -> CreateInventoryCommand:
    return CreateInventoryCommand(aggregate=aggregate, event_repository=event_repository)
...
async def get_update_inventory_command(
    ...
    return UpdateInventoryCommand(aggregate=aggregate, event_repository=event_repository)
...
async def get_complete_reserved_command(
    ...
    return CompleteReservedStockCommand(aggregate=aggregate, event_repository=event_repository)


async def get_inventory_query(repository: IInventoryRepository = Depends(inventory_event_repository)) -> GetInventoryQuery:
