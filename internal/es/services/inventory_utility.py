...
class MongoDBInventoryUtility(IEventSourcingUtility):
    @staticmethod
    async def recreate_state(
        repository: IInventoryRepository, sku: str
    ) -> Inventory | None:
            ...
            match event.get("event_type"):
                ...
                case InventoryEventType.AVAILABLE_QUANTITY_DECREASED:
                    inventory.update_available_quantity(
                        amount=int(event.get("available_quantity"))
                    )
                case InventoryEventType.AVAILABLE_QUANTITY_REPLACED:
                    inventory.set_available_quantity(
                        available_quantity=int(event.get("available_quantity"))
                    )
                ...
                case InventoryEventType.PROCESSED_RESERVED_SOH_DECREASED:
                    inventory.update_soh(amount=int(event.get("soh")))
                case InventoryEventType.PROCESSED_RESERVED_DECREASED:
                    inventory.decrease_reserved(amount=int(event.get("reserved")))
        if inventory.sku == "":
            return None
        return inventory
