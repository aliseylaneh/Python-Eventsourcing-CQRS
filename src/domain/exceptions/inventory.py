...
class InvalidRelatedEventType(Exception):
    ...
    def __init__(self, event_type: Enum, aggregate):
        super(InvalidRelatedEventType, self).__init__(
            f"Event type {event_type} isn't related to aggregate {aggregate.__class__}."
        )
...
class AvailableQuantityError(Exception):
    def __init__(self, *args, **kwargs):
        super(AvailableQuantityError, self).__init__(
            "Available Quantity is more than soh."
        )


class ReservedStockInProcess(Exception):
    def __init__(self, *args, **kwargs):
        super(ReservedStockInProcess, self).__init__(
            "Update is unavailable due to reserved stock."
        )


class InvalidReservedAmount(Exception):
    def __init__(self, *args, **kwargs):
        super(InvalidReservedAmount, self).__init__(
            "Amount is more than reserved quantity."
        )
